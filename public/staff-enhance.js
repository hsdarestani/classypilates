(()=>{
  ...
  const api=async(path,opt={})=>{const h={...(opt.headers||{}),authorization:`Bearer ${token()}`};if(!(opt.body instanceof FormData))h['content-type']='application/json';const r=await fetch(path,{...opt,headers:h});const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.detail||'Error');return d};
  ...
  function modal(c){
    ...
    w.innerHTML=`<div class="panel" style="width:min(650px,100%);margin:0"><div class="panel-head"><div><p class="kicker">EDIT CLASS</p><h2>Edit class</h2><p>${esc(c.studio_name)}</p></div><button class="secondary" data-close>×</button></div><div class="form-grid"><label>CLASS<input data-title value="${esc(c.name)}"></label><label>TYPE<select data-type><option ${c.type==='Reformer'?'selected':''}>Reformer</option><option ${c.type==='Powerformer'?'selected':''}>Powerformer</option><option ${c.type==='Mat'?'selected':''}>Mat</option><option ${c.type==='Barre'?'selected':''}>Barre</option></select></label><label>START<input data-start type="datetime-local" value="${value}"></label><label>DURATION<input data-duration type="number" value="${c.duration}"></label><label>CAPACITY<input data-capacity type="number" value="${c.capacity}"></label></div><button class="primary" data-save>Save changes</button><div class="msg" data-msg></div></div>`;
  ...
  async function enhance(){
    ...
    buttons.forEach(del=>{if(del.parentElement.querySelector('[data-edit-class]'))return;const id=Number(del.dataset.deleteClass),c=cache.find(x=>x.id===id);if(!c)return;const b=document.createElement('button');b.className='secondary';b.dataset.editClass=id;b.textContent='Edit';b.onclick=()=>modal(c);del.parentElement.insertBefore(b,del)});
  }
