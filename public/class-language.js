(()=>{
  ...
  style.textContent=`
    .class-name{position:relative;padding-right:62px}
    .class-name .class-language{position:absolute;right:0;top:50%;transform:translateY(-50%);display:inline-flex;align-items:center;justify-content:center;min-width:42px;width:max-content;padding:7px 10px;margin:0;border:1px solid rgba(21,21,19,.14);border-radius:999px;background:#f4f0e8;color:#454139;font:700 9px/1 Manrope,Arial,sans-serif;letter-spacing:.08em;white-space:nowrap;z-index:2}
    ...
    @media(max-width:760px){.class-name{padding-right:0}.class-name .class-language{position:static;transform:none;margin-top:8px;padding:6px 9px}}
  `;
  ...
  function decorate(){
    document.querySelectorAll('#classList .class-row').forEach(row=>{
      ...
      badge.innerHTML=`<span class="class-language-code">${language.toUpperCase()}</span>`;
    });
  }
