(()=>{
  ...
  async function authenticateCustomer(){const details=wizard.details;let result;if(wizard.mode==='first'){try{result=await accountRequest('/api/auth/register',{email:details.email,password:details.password,first_name:details.firstName,last_name:details.lastName,phone:details.phone,birth_date:details.birth,emergency_contact:details.emergency,marketing_opt_in:details.news})}catch(error){if(error.message!=='email_exists')throw error;result=await accountRequest('/api/auth/login',{email:details.email,password:details.password})}}else{result=await accountRequest('/api/auth/login',{email:details.email,password:details.password})}if(result.user?.portal!=='/account')throw new Error('customer_account_required');return result.user}
  ...
  function renderSuccess(ref){const method=payMethods.find(x=>x.id===wizard.payment)?.name||wizard.payment;setDrawer('Booking confirmed',`<div class="booking-success-v2"><div class="success-orbit"><span>✓</span></div><p class="eyebrow">YOU'RE IN</p><h3>See you in class.</h3><p class="success-lead">Dein Platz ist reserviert und mit deinem Classy-Konto verbunden.</p>${classSummary(wizard.class)}<div class="success-grid"><div><span>SPOT</span><b>${safe(spotLabel(wizard.spot,wizard.class))}</b></div><div><span>PAYMENT</span><b>${safe(method)}</b></div><div><span>BOOKING</span><b>${safe(ref)}</b></div></div><div class="demo-payment-note success"><span>ACCOUNT READY</span><p>Buchungen, Credits und Profil findest du ab jetzt in „Mein Classy“.</p></div><div class="success-actions"><a class="drawer-action" href="/account">Mein Classy öffnen</a><a class="drawer-action secondary" href="${calendarHref()}" download="classy-pilates.ics">Zum Kalender</a><button class="drawer-action secondary" id="doneV2">Fertig</button></div></div>`,5);$('#doneV2')?.addEventListener('click',closeDrawer)}
