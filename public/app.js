...
function openClass(r){
  ...
  if(full){
    $('#drawerBody').innerHTML=selectedSummary(r)+`<div class="drawer-step"><h4>We will let you know when a spot opens.</h4><p>Enter your details. The waitlist is confirmed live with Mindbody and can be removed under “My bookings”.</p><label class="field"><span>FIRST NAME</span><input id="waitFirstName" type="text" autocomplete="given-name" placeholder="First name"></label><label class="field"><span>LAST NAME</span><input id="waitLastName" type="text" autocomplete="family-name" placeholder="Last name"></label><label class="field"><span>EMAIL</span><input id="waitEmail" type="email" autocomplete="email" placeholder="name@email.de"></label><button class="drawer-action" id="joinWaitlist" type="button">Join waitlist</button><button class="drawer-action secondary" id="cancelBooking" type="button">Cancel</button></div>`;
