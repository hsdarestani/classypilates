(()=>{
  ...
  function showApp(){
    ...
    const isCoach=state.user.portal==='/coach';
    document.body.classList.toggle('coach-portal',isCoach);
    document.body.classList.toggle('admin-portal',!isCoach);
    ...
    const portalMode=$('#portalMode');if(portalMode)portalMode.textContent=isCoach?'COACH SPACE':'ADMIN CONTROL';
  ...
  async function dashboard(){
    ...
    const isCoach=state.user?.portal==='/coach'||d.scope==='coach';
    let recent=[];if(has('bookings.view'))try{recent=(await api('/api/staff/bookings')).bookings.slice(0,6)}catch(_){}
    const firstName=state.user?.first_name||state.user?.coach?.display_name||'';
    const hero=isCoach
      ? `<section class="portal-hero coach-hero"><div><p class="kicker">COACH WORKSPACE</p><h2>${firstName?`Welcome, ${esc(firstName)}.`:'Your coaching day.'}</h2><p>Your schedule and attendees are scoped to your own classes. Mindbody remains the source of truth for live class data.</p></div><div class="portal-hero-actions"><button class="secondary" id="heroSchedule">My schedule</button><button class="primary compact" id="heroProfile">My profile</button></div></section>`
      : '<section class="portal-hero admin-hero"><div><p class="kicker">LIVE OPERATIONS</p><h2>Classy Control Center</h2><p>Bookings, customers, studios, coaches and revenue in one production workspace.</p></div><div class="portal-hero-status"><span><i></i> PRODUCTION LIVE</span><small>Mindbody + SumUp connected workflows</small></div></section>';
  ...
  async function coachManager(){
    ...
    $$('[data-coach-account]').forEach(button=>{const coach=data.coaches.find(item=>item.id===Number(button.dataset.coachAccount));button.onclick=()=>coachAccountModal(coach)});
    $$('[data-coach-reset]').forEach(button=>{const coach=data.coaches.find(item=>item.id===Number(button.dataset.coachReset));button.onclick=()=>coachPasswordResetModal(coach)});
  }

  async function profile(){
    const coach=await api('/api/staff/profile');
    $('#view').innerHTML=`<section class="panel coach-profile-layout"><div class="coach-profile-photo">${coachAvatar(coach,true)}<button class="primary" id="profilePhoto">${coach.photo_url?'Change photo':'Add profile photo'}</button>${coach.photo_url?'<button class="danger" id="profilePhotoRemove">Remove photo</button>':''}<small>JPG, PNG or WebP · maximum 5 MB</small></div><div class="coach-profile-form"><p class="kicker">PUBLIC PROFILE</p><h2>Your coach profile</h2><p>This name, bio and photo can appear in the public schedule and booking experience.</p><label>DISPLAY NAME<input id="profileName" value="${esc(coach.display_name)}"></label><label>BIO<textarea id="profileBio" rows="7" maxlength="2000" placeholder="Tell clients about your training style and experience.">${esc(coach.bio||'')}</textarea></label><button class="primary" id="profileSave">Save profile</button><div class="profile-security"><p class="kicker">SECURITY</p><h3>Change password</h3><label>CURRENT PASSWORD<input id="currentPassword" type="password" autocomplete="current-password"></label><label>NEW PASSWORD<input id="newPassword" type="password" autocomplete="new-password" placeholder="At least 8 characters"></label><label>CONFIRM NEW PASSWORD<input id="confirmPassword" type="password" autocomplete="new-password"></label><label class="password-toggle"><input type="checkbox" data-password-toggle="#currentPassword,#newPassword,#confirmPassword"> Show passwords</label><button class="secondary" id="passwordSave">Update password</button></div></div></section>`;
    ...
    $('#passwordSave').onclick=async()=>{const current=$('#currentPassword').value,next=$('#newPassword').value,confirmPassword=$('#confirmPassword').value;if(next.length<8)return toast('Use at least 8 characters');if(next!==confirmPassword)return toast('New passwords do not match');try{await api('/api/staff/change-password',{method:'POST',body:JSON.stringify({current_password:current,new_password:next})});toast('Password updated');$('#currentPassword').value=$('#newPassword').value=$('#confirmPassword').value=''}catch(error){toast(error.message==='current_password_invalid'?'Current password is incorrect':error.message)}};
  }
